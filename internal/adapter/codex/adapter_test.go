...
func TestParseSession(t *testing.T) {
	a := &Adapter{}
	result, err := a.ParseSession(testDataPath(t, "codex/session.jsonl"))
	if err != nil {
...
func TestDiscover(t *testing.T) {
	a := &Adapter{}
	files, err := a.Discover(testDataPath(t, "codex"))
	if err != nil {
...
func TestAdapterID(t *testing.T) {
	a := &Adapter{}
	if a.ID() != adapter.AgentCodex {
		t.Errorf("expected %s, got %s", adapter.AgentCodex, a.ID())
	}
}
